...
import { 
  ...
  Briefcase, ShieldCheck, Trophy, Clock
} from 'lucide-react';
...
export default function PanelPage() {
  ...
  const estadisticas = [
    { nombre: 'Pacientes Activos', valor: kpis.pacientes_activos || '0', icono: Users, colorIcono: 'text-blue-600', fondoIcono: 'bg-blue-50' },
    { nombre: 'Visitas Hoy', valor: sumVisitasHoy.toString(), icono: Calendar, colorIcono: 'text-emerald-600', fondoIcono: 'bg-emerald-50' },
    { nombre: 'Órdenes Vigentes', valor: kpis.ordenes_vigentes || '0', icono: FileText, colorIcono: 'text-purple-600', fondoIcono: 'bg-purple-50' },
    { nombre: 'Tutelas Activas', valor: kpis.tutelas_activas || '0', icono: Activity, colorIcono: 'text-red-600', fondoIcono: 'bg-red-50' },
  ];

  const laboratoriosPendientes = data.gestion_recursos?.laboratorios_pendientes || [];
  const topDiagnosticos = data.demografia?.top_diagnosticos || [];
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-700 pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        {/* Tabla de Laboratorios Pendientes */}
        <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden pt-6 flex flex-col hover:shadow-md transition-all">
          <div className="px-6 mb-5 flex items-center justify-between">
            <h3 className="font-black text-gray-900 text-lg">Laboratorios Pendientes</h3>
            <button className="text-[11px] font-black uppercase tracking-wider text-[#2563EB] hover:text-[#1E40AF] transition-colors border border-blue-100 px-3 py-1 rounded-lg bg-blue-50/30">Ver todos</button>
          </div>
          <div className="overflow-x-auto flex-1 max-h-[350px] overflow-y-auto custom-scrollbar">
            <table className="w-full relative">
              <thead className="bg-[#F9FAFB] border-y border-gray-100 sticky top-0 z-10 shadow-sm">
                <tr>
                  <th className="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest text-left">Paciente</th>
                  <th className="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest text-left">Servicio</th>
                  <th className="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest text-left">Hora Programada</th>
                  <th className="px-6 py-4 text-[10px] font-black text-gray-400 uppercase tracking-widest text-left">Estado</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {laboratoriosPendientes.map((lab) => {
                  const date = new Date(lab.fecha_toma_programada);
                  const hora = date.toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit' }) !== 'Invalid Date' 
                        ? date.toLocaleTimeString('es-CO', { hour: '2-digit', minute: '2-digit' }) 
                        : (lab.fecha_toma_programada?.split(' ')[1]?.substring(0,5) || '00:00');

                  return (
                    <FilaVisita 
                      key={lab.id_laboratorio} 
                      nombre={lab.paciente}
                      servicio={`Laboratorio #${lab.id_laboratorio}`}
                      hora={hora}
                      estado="PENDIENTE"
                      colorEstado="text-amber-600 bg-amber-50 border border-amber-100"
                    />
                  );
                })}
                {laboratoriosPendientes.length === 0 && (
                  <tr>
                    <td colSpan="4" className="text-center py-12 text-gray-400 text-xs font-black uppercase tracking-widest">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 flex flex-col hover:shadow-md transition-all">
          <h3 className="font-black text-gray-900 text-lg mb-6">Top Diagnósticos</h3>
          <div className="space-y-6 flex-1 max-h-[350px] overflow-y-auto custom-scrollbar px-1">
            {topDiagnosticos.map((diag, indice) => (
              <ItemActividad 
                key={indice} 
                etiqueta={diag.descripcion} 
                tiempo={`${diag.total_casos} casos detectados`} 
                icono={Activity} 
                color="bg-red-50 text-red-600 border border-red-100" 
              />
            ))}
            {topDiagnosticos.length === 0 && (
              <p className="text-center text-gray-400 text-xs font-black py-8 uppercase tracking-widest opacity-40">No hay datos disponibles</p>
            )}
          </div>
        </div>
      </div>
